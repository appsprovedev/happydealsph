<?php
/**
 * Plugin Name:Happy Deals Email Verification
...
class woocommerce_confirmation_email {
    ...
    public function __construct() {
        ...
        add_shortcode('wcemailverificationuser', array($this, 'wc_email_verification_user'));
    ...
    public function new_user_registeration_resend($customer, $user_id) {
        $current_user = get_user_by('id', $user_id);
        $this->user_id = $current_user->ID;
        $this->email_id = $current_user->user_email;
        $scret_code = md5($this->user_id . time());
    ...
    function please_confirm_email_message() {
        ...
        <ul class="woocommerce-info">
            <li><strong>Thank you for signing up with us, pls check your email to verify your account.</strong> Did not receive an email? <a style='font-size:14px;color:red 'href="<?php echo $link ?>">Resend verification email.</a></li>
        </ul>
        <?php
    }
    ...
    function authenticate_user_by_email() {
        if (isset($_GET["woo_confirmation_verify"])) {
            ...
            if (get_user_meta((int) $user_meta[1], "wcemailverifiedcode", TRUE) == $user_meta[0]) {     
                if (!get_user_meta((int) $user_meta[1], "wcemailverified",true)) {
        ...
        if (isset($_GET["wc_confirmation_resend"])) {
            ...
            $this->new_user_registeration_resend(1, $user_id);
        }
    }
    ...
    public function  wc_email_verification_user() {
