	...
	<div class="isotope-wrapper" data-isotope-gutter="5" data-isotope-cols="3" data-isotope-cols-sm="2">
		...
		if ( ! empty( $terms ) && ! is_wp_error( $terms ) ){
			...
			foreach ($companies_ids as $company_id) {
				...
				if ( ! empty( $current_company ) && ! is_wp_error( $current_company ) && couponhut_get_field('company_logo', 'deal_company_' . $company_id) ){
					$image = couponhut_get_field('company_logo', 'deal_company_' . $company_id);
