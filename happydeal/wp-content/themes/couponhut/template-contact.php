...
<div class="page-contact-wrapper">
	...
	<div class="split-section-row page-contact-map" data-split="1/1">
		<div class="split-section-col">
				<?php if ( couponhut_get_field('image_or_map_location') == 'image' ): ?>
					<?php $image = couponhut_get_field('upload_image'); ?>
					<div class="bg-image" data-bgimage="<?php echo esc_url( $image['sizes']['ssd_half-image'] ); ?>"></div>
					<div class="overlay-dark"></div>
					<div class="split-section-content">
						<h1 class="light-text"><?php echo wp_kses_post(couponhut_get_field('image_title')) ?></h1>
						<?php echo wp_kses_post(couponhut_get_field('image_text')) ?>
					</div>
				<?php else: ?>
					<?php $location = couponhut_get_field('location'); ?>
