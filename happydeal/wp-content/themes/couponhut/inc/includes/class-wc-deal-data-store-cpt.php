...
/**
 * WC Deal Data Store: Stored in CPT.
 *
 * @version  3.0.0
 * @category Class
 * @author   WooThemes
 */
class WC_Deal_Data_Store_CPT extends WC_Data_Store_WP implements WC_Object_Data_Store_Interface, WC_Product_Data_Store_Interface {
	...
	*/
	...
	/**
	 * Method to create a new deal in the database.
	 *
	 * @param WC_CouponHut_Deal
	 */
	public function create( &$product ) {
		...
		$id = wp_insert_post( apply_filters( 'woocommerce_new_product_data', array(
			...
			'post_title'     => $product->get_name() ? $product->get_name() : __( 'Deal', 'couponhut' ),
	...
	public function read( &$product ) {
		...
		if ( ! $product->get_id() || ! ( $post_object = get_post( $product->get_id() ) ) || 'deal' !== $post_object->post_type ) {
			throw new Exception( __( 'Invalid deal.', 'couponhut' ) );
		}
	...
	public function delete( &$product, $args = array() ) {
		$id        = $product->get_id();
		$post_type = $product->is_type( 'variation' ) ? 'product_variation' : 'deal';
	...
	*/
	...
	/**
	 * Helper method that updates all the post meta for a product based on it's settings in the WC_CouponHut_Deal class.
	 *
	 * @param WC_CouponHut_Deal
	 ...
	 *
	 * @since  3.0.0
	 * @param  WC_CouponHut_Deal $product
	 */
	...
	*/
	...
	public function get_on_sale_products() {
		...
		return $wpdb->get_results( "
			...
			WHERE post.post_type IN ( 'deal', 'product_variation' )
	...
	public function get_featured_product_ids() {
		...
		return get_posts( array(
			'post_type'      => array( 'deal', 'product_variation' ),
			'posts_per_page' => -1,
			'post_status'    => 'publish',
			'tax_query'      => array(
				'relation' => 'AND',
	...
	public function is_existing_sku( $product_id, $sku ) {
		...
		return $wpdb->get_var( $wpdb->prepare( "
			...
			WHERE $wpdb->posts.post_type IN ( 'deal', 'product_variation' )
	...
	public function get_product_id_by_sku( $sku ) {
		...
		return $wpdb->get_var( $wpdb->prepare( "
			...
			WHERE posts.post_type IN ( 'deal', 'product_variation' )
	 ...
	 *
	 * @since  3.0.0
	 * @param  WC_CouponHut_Deal $product Variable product.
	...
	public function get_related_products_query( $cats_array, $tags_array, $exclude_ids, $limit ) {
		...
		$query['where'] .= " AND p.post_type = 'deal'";
	...
	public function get_products( $args = array() ) {
		 ...
		 */
		$wp_query_args = array(
			'post_type'      => 'variation' === $args['type'] ? 'product_variation' : 'deal',
	...
	public function search_products( $term, $type = '', $include_variations = false ) {
		...
		$post_types    = $include_variations ? array( 'deal', 'product_variation' ) : array( 'deal' );
