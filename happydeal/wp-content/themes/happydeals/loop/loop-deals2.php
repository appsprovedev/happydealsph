...
if ( !empty($_GET['deal_search']) ) {
	$args['s'] = $_GET['deal_search'];
}
...
if ( !empty($_GET['sort']) ) {
	...
	if ( $_GET['sort'] == 'viewed' ) {
		$args['meta_key'] = 'ssd_post_views_count';
		$args['orderby'] = 'meta_value_num';
		$args['order'] = 'desc';
	}
...
if ( !empty($_GET['deal_country']) ) {
	...
	$country_slug = sanitize_title($_GET['deal_country']);
...
if ( !empty($_GET['deal_city']) ) {
	...
	$city_slug = sanitize_title($_GET['deal_city']);
	...
	$args['meta_query']['relation'] = 'AND';
	array_push($args['meta_query'], $city_array);
}
	...
	*/
	...
	<div class="grid-wrapper is-ajax-deals-content">
		...
		<?php while  ( $deals_query->have_posts() ) : $deals_query->the_post(); ?>
			...
			<div class="deal-item-wrapper col-xs-6 col-sm-6 col-md-3">
